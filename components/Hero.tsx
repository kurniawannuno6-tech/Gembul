...
export default function Hero({ loaded }: { loaded: boolean }) {
  ...
  useEffect(() => {
    gsap.set([eyebrowRef.current, line1Ref.current, line2Ref.current, subRef.current, scrollRef.current], { opacity: 0 });
    gsap.set([line1Ref.current, line2Ref.current], { y: 60 });
    gsap.set([eyebrowRef.current, subRef.current, scrollRef.current], { y: 20 });
